...
class PAACRunner:
    ...
    def _create_shared_transitions(self):
        state = self._get_shared(
            np.zeros(
                (self.num_envs,) + self.env[0].observation_space.shape, dtype="float"
            )
        )
    ...
    def _get_action_array(self):
        shape = (self.num_envs, np.prod(self.env[0].action_space.shape))
    ...
    def sample_random_action(self):
        return np.array([env.action_space.sample() for env in self.env])
